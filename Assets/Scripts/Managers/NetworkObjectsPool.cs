...
public class NetworkObjectsPool : NetworkSingleton<NetworkObjectsPool>
{
    [SerializeField]
    ...
    {
        if (PooledPrefabsList.Count == 0) return;
        ...
        for (var i = 0; i < PooledPrefabsList.Count; i++)
        {
            ...
            if (prefab != null)
            {
                Assert.IsNotNull(prefab.GetComponent<NetworkObject>(), $"{nameof(NetworkObjectsPool)}: Pooled prefab \"{prefab.name}\" at index {i.ToString()} has no {nameof(NetworkObject)} component.");
            }
        }
    }
...
{
    GameObject m_Prefab;
    NetworkObjectsPool m_Pool;

    public PooledPrefabInstanceHandler(GameObject prefab, NetworkObjectsPool pool)
    {
        m_Prefab = prefab;
        m_Pool = pool;
    }
